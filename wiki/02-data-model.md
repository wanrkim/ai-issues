...
```json
{
  ...
  "next_id": 26,
  "baselines": { "llm": 4.358, "media": 4.721, "hardware": 1.924, "capital": 3.539 },
  "issues": [
    {
      "id": "iss_000001",
      "rank": 1,
...
| `baselines` | 객체 | 축별 기준선. 점수를 계산할 때 출처 수를 이 값으로 나눈다. 계산 방법은 [05-ranking.md](05-ranking.md) |
...
| `company_domain` | 문자열 또는 null | 이 소식의 중심에 있는 기업의 대표 도메인. 사진이 없을 때 로고를 그리는 데 쓴다 |
| `image` | 문자열 또는 null | 섬네일 주소. 언론사 서버의 이미지를 그대로 참조한다 |
| `image_tried` | 배열 | 이미지를 찾으려고 이미 열어본 원문 주소. 같은 주소를 반복해서 열지 않기 위해 저장한다 |
| `summary` | 문자열 | 한국어 세 문장에서 네 문장. 200자 안팎. 항목을 펼쳤을 때 보여준다. 이 필드를 넣기 전에 만들어진 이슈는 빈 문자열이다 |
| `impact` | 정수 | 1에서 5 사이. 점수 계산에는 쓰지 않는다 |
| `score` | 실수 | 순위 점수. 소수점 셋째 자리에서 반올림한다 |
| `source_count` | 정수 | 이 이슈에 묶인 글의 수 |
| `first_seen` | 문자열 | 이슈가 처음 제기된 시각. 화면에 표시하고 정렬 기준으로 쓴다. 가장 이른 글이 보관 기간을 넘겨 사라져도 앞으로 밀리지 않는다 |
