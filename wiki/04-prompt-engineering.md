...
`{live_issues}`는 `id | 제목 | 부제 | 축` 형식의 줄 목록으로 바꾼다. 기업 도메인이 비어 있는 이슈는 줄 끝에 `[도메인 없음]`을 붙인다. 이슈가 없으면 `(없음)`을 넣는다.
`{new_items}`는 `번호 | 출처 | 제목 | 요약 일부` 형식의 줄 목록으로 바꾼다.

## 임팩트 기준표

임팩트는 순위 점수에 쓰지 않는다. 값은 저장하고 실행 기록에 표시한다. 점수 식은 [05-ranking.md](05-ranking.md)에 기록한다.
...
이슈마다 중심이 되는 기업의 도메인을 함께 받는다. 화면에서 기사 사진이 없는 이슈에 그 기업의 로고를 보여주는 데 쓴다.

기존 이슈 중 도메인이 비어 있는 것은 같은 호출의 `domains` 배열로 채운다. 제목과 부제만 보고 판단할 수 있으므로 원문이 필요 없고, 호출 횟수도 늘지 않는다.
